...
function sendTomorrowOverviewToShmulikFromCloud() {
  try {
    ...
    // פונקציית עזר למניעת כפילות של אותו כלב ברישום כפול
    var dedupeDogList = function(arr) {
      var map = {};
      var out = [];
      for (var dIdx = 0; dIdx < arr.length; dIdx++) {
        var it = arr[dIdx];
        var key = ((it.owner_phone || "").replace(/[^0-9]/g, "")) + "_" + ((it.dog_name || "").trim().toLowerCase());
        if (!map[key]) {
          map[key] = true;
          out.push(it);
        }
    ...
    incomingDogs = dedupeDogList(incomingDogs);
    departingDogs = dedupeDogList(departingDogs);
    endOfDayDogs = dedupeDogList(endOfDayDogs);
    daytimeDogs = dedupeDogList(daytimeDogs);
...
function sendTomorrowOverviewToShmulikFromCloud() {
  try {
    ...
    var isTrainingBk = function(b) {
      var s = b.service_type || b.serviceType || "";
      return s === "training" || s === "day_training" || s === "combined";
    };

    var incomingBoarding = incomingDogs.filter(function(b) { return !isTrainingBk(b); });
    var incomingTraining = incomingDogs.filter(function(b) { return isTrainingBk(b); });

    var departingBoarding = departingDogs.filter(function(b) { return !isTrainingBk(b); });
    var departingTraining = departingDogs.filter(function(b) { return isTrainingBk(b); });

    var endOfDayBoarding = endOfDayDogs.filter(function(b) { return !isTrainingBk(b); });
    var endOfDayTraining = endOfDayDogs.filter(function(b) { return isTrainingBk(b); });

    var daytimeBoarding = daytimeDogs.filter(function(b) { return !isTrainingBk(b); });
    var daytimeTraining = daytimeDogs.filter(function(b) { return isTrainingBk(b); });
    ...
    var formatDogAppScript = function(bk, idx, isIncoming) {
      ...
      if (!isPaid && debt > 0 && phone) {
        ...
        var fName = (owner.split(" ")[0] || "לקוח");
        var isFemale = (bk.dog_gender && bk.dog_gender.indexOf("female") !== -1);
        var stayDesc = isTr ? "תהליך האילוף" : "השהות בריזורט";

        var demandMsg = "";
        if (isIncoming) {
          demandMsg = "היי " + fName + "! 🐾\nמתרגשים ומחכים מחר לתחילת " + stayDesc + " של " + dog + " בריזורט לכלב! 🐶❤️\n\nלקראת ההגעה מחר, נשמח להסדרת יתרת התשלום בסך ₪" + debt.toLocaleString() + ".\nלתשלום מהיר, נוח ומאובטח ב-Bit או כרטיס אשראי:\n👉 " + growLink + "\n\nמחכים לכם בשמחה,\nשמוליק וצוות הריזורט לכלב 🐾✨";
        } else {
          var fVerb = isFemale ? "מסיימת" : "מסיים";
          var eVerb = isFemale ? "נהנתה" : "נהנה";
          var mVerb = isFemale ? "מתגעגעת" : "מתגעגע";
          demandMsg = "היי " + fName + "! 🐾\nרצינו לעדכן שמחר " + dog + " " + fVerb + " את " + stayDesc + " בריזורט לכלב! 🐕🥰 " + eVerb + " מכל רגע ו" + mVerb + " אליכם מאוד.\n\nלקראת האיסוף והשחרור מחר, נשמח להסדרת יתרת התשלום בסך ₪" + debt.toLocaleString() + ".\nלתשלום מהיר, נוח ומאובטח ב-Bit או כרטיס אשראי:\n👉 " + growLink + "\n\nתודה רבה ונתראה מחר,\nשמוליק וצוות הריזורט לכלב 🐾✨";
        }
    ...
    if (incomingDogs.length === 0) {
      inSec = "• אין כניסות מתוכננות למחר (0 פנסיון | 0 אילוף).";
    } else {
      var incArr = [];
      incArr.push("🏨 *כניסות לפנסיון (" + incomingBoarding.length + "):*");
      if (incomingBoarding.length > 0) {
        incArr.push(incomingBoarding.map(function(d, i) { return formatDogAppScript(d, i, true); }).join("\n\n"));
      } else {
        incArr.push("• אין כניסות לפנסיון מחר.");
      }
      incArr.push("\n🎓 *כניסות לאילוף (" + incomingTraining.length + "):*");
      if (incomingTraining.length > 0) {
        incArr.push(incomingTraining.map(function(d, i) { return formatDogAppScript(d, i, true); }).join("\n\n"));
      } else {
        incArr.push("• אין כניסות לאילוף מחר.");
      }
      inSec = incArr.join("\n");
    }
    ...
    if (departingDogs.length === 0) {
      outSec = "• אין שחרורים מתוכננים למחר (0 פנסיון | 0 אילוף).";
    } else {
      var depArr = [];
      depArr.push("🏨 *שחרורים מפנסיון (" + departingBoarding.length + "):*");
      if (departingBoarding.length > 0) {
        depArr.push(departingBoarding.map(function(d, i) { return formatDogAppScript(d, i, false); }).join("\n\n"));
      } else {
        depArr.push("• אין שחרורים מפנסיון מחר.");
      }
      depArr.push("\n🎓 *שחרורים מאילוף (" + departingTraining.length + "):*");
      if (departingTraining.length > 0) {
        depArr.push(departingTraining.map(function(d, i) { return formatDogAppScript(d, i, false); }).join("\n\n"));
      } else {
        depArr.push("• אין שחרורים מאילוף מחר.");
      }
      outSec = depArr.join("\n");
    }
    ...
    var boardingNames = endOfDayBoarding.map(function(b) { return b.dog_name; }).filter(Boolean);
    var trainingNames = endOfDayTraining.map(function(b) { return b.dog_name; }).filter(Boolean);

    var bLine = endOfDayBoarding.length > 0
      ? "• 🏨 *פנסיון ללינה (" + endOfDayBoarding.length + "):* " + boardingNames.join(", ")
      : "• 🏨 *פנסיון ללינה:* 0 כלבים";

    var tLine = endOfDayTraining.length > 0
      ? "• 🎓 *אילוף ללינה (" + endOfDayTraining.length + "):* " + trainingNames.join(", ")
      : "• 🎓 *אילוף ללינה:* 0 כלבים";
    ...
    var fullMsg = "📋 *מה קורה מחר? סקירה יומית לשמוליק – הריזורט לכלב* 🐾\n"
      ...
      + tLine + "\n"
      + "━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
      + "📊 *סה״כ כלבים שנמצאים מחר (במהלך היום): " + daytimeDogs.length + " כלבים*\n"
      + "• 🏨 פנסיון ויומיות: *" + daytimeBoarding.length + " כלבים*\n"
      + "• 🎓 תהליכי אילוף: *" + daytimeTraining.length + " כלבים*\n\n"
