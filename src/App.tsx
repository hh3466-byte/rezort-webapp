...
• הפרש: ₪0 (הכול מאוזן ונסגר ל-0)`}
        ...
        {/* =========================================================================
            MASTER NAVIGATION & OPERATIONS COMMAND CENTER (VISUAL & SYMMETRIC)
           ========================================================================= */}
        <div className="space-y-2.5 my-1.5 select-none">
          
          {/* TIER 1: MAIN APPLICATION NAVIGATION TABS & MASTER ACTIONS */}
          <div className="bg-white/95 backdrop-blur-md rounded-2xl border border-slate-200 shadow-xs p-2 sm:p-2.5 flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-2.5 w-full">
            
            {/* Right Side in RTL (מימין): Main Navigation Tabs */}
            <div className="flex items-center bg-slate-100/90 p-1 rounded-xl border border-slate-200/80 gap-1 overflow-x-auto no-scrollbar shrink-0">
              {/* Calendar Tab */}
              <button
                type="button"
              ...
              {/* Kennels Placement & Feeding Schedule Tab */}
              <button
                type="button"
              ...
              <button
                ...
                className={`text-xs sm:text-sm font-black px-3.5 sm:px-4 py-2 rounded-lg transition-all cursor-pointer flex items-center gap-1.5 shrink-0 ${
                  activeTab === 'whatsapp'
                    ? 'bg-[#065f46] text-white shadow-xs ring-2 ring-emerald-400'
                    : newCrmChatsCount > 0
                    ? 'bg-rose-50 text-rose-950 hover:bg-rose-100 font-black'
                    : 'text-slate-600 hover:text-slate-900 hover:bg-white/80'
                }`}
                title="מרכז וואטסאפ ופניות (CRM) – ניהול שיחות ומענה מהיר"
              >
                <span className="text-base">💬</span>
                <span>וואטסאפ ו-CRM</span>
                {newCrmChatsCount > 0 ? (
                  <span className="bg-rose-600 text-white text-[10px] font-black px-1.5 py-0.2 rounded-full shadow-2xs animate-pulse">
                    {newCrmChatsCount}
                  </span>
                ) : (
            ...
            {/* Center Area: Shabbat / Jewish Holiday Dog Greetings Reminder Banner (if active) */}
            {todayHolidayInfo.isSpecial && totalDogsToday > 0 && !isGreetingBannerDismissed && (() => {
              ...
              if (rest.isRestricted) {
                return (
                  <div className="flex-1 min-w-[200px] mx-1 bg-gradient-to-r from-amber-600 via-orange-600 to-amber-700 text-white rounded-xl px-2.5 sm:px-3 py-1.5 shadow-xs border border-amber-400/60 flex items-center justify-between gap-2 animate-in fade-in duration-200">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="text-base shrink-0">🛡️</span>
                      <span className="font-black text-xs tracking-tight truncate">
                        שקט מוחלט ללקוחות (שישי/שבת/חג)
                      </span>
                    </div>
                    <button
                      type="button"
                      onClick={() => setGreetingModalDate(todayStr)}
                      className="bg-white/95 hover:bg-white text-slate-900 font-black px-2.5 py-1 rounded-lg text-xs shadow-xs transition-all cursor-pointer shrink-0"
                    >
                      צפה ברשימה
                    </button>
                  </div>
              ...
              if (todayUnsentGreetingsCount > 0) {
                return (
                  <div className="flex-1 min-w-[200px] mx-1 bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-700 text-white rounded-xl px-2.5 sm:px-3 py-1.5 shadow-xs border border-emerald-400/60 flex items-center justify-between gap-2 animate-in fade-in duration-200">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="text-base shrink-0">🚀</span>
                      <span className="font-black text-xs tracking-tight truncate">
                        מוצאי שבת – ד״ש פעיל ({todayUnsentGreetingsCount} להשלמה)
                      </span>
                    </div>
                    <button
                      type="button"
                      onClick={() => setGreetingModalDate(todayStr)}
                      className="bg-white hover:bg-emerald-50 text-emerald-950 font-black px-2.5 py-1 rounded-lg text-xs shadow-xs transition-all cursor-pointer shrink-0"
                    >
                      צפה ברשימה
                    </button>
                  </div>
            ...
            {/* Left Side in RTL (משמאל): Primary CTA (+ הזמנה חדשה) + System Utilities */}
            <div className="flex items-center justify-end gap-2 shrink-0">
              {/* Primary CTA: + הזמנה חדשה */}
              <button
                onClick={() => setBookingWizardOpen({ isOpen: true, initialData: null })}
                id="btn-new-booking-top"
                className="bg-[#065f46] hover:bg-[#044e45] active:scale-95 text-white font-black px-4 py-2 rounded-xl text-xs sm:text-sm shadow-xs hover:shadow-md ring-2 ring-emerald-500/30 flex items-center gap-1.5 transition-all cursor-pointer shrink-0"
                title="פתיחת אשף הזמנה חדשה"
              >
                <span className="text-base font-bold">+</span>
                <span>הזמנה חדשה</span>
              </button>

              {/* Segmented System Utilities Container */}
              <div className="flex items-center bg-slate-100/90 p-1 rounded-xl border border-slate-200/80 gap-1 shrink-0">
                <button
                  type="button"
                  onClick={() => setIsReportsOpen(true)}
                  className="text-xs sm:text-sm font-bold text-slate-700 hover:text-slate-950 hover:bg-white px-3 py-1.5 rounded-lg transition-all cursor-pointer flex items-center gap-1.5 shrink-0 shadow-2xs"
                ...
                <button
                  ...
                  className="text-xs sm:text-sm font-bold text-slate-500 hover:text-rose-700 hover:bg-rose-50 px-2.5 py-1.5 rounded-lg transition-all cursor-pointer flex items-center gap-1 shrink-0 group"
          ...
          {/* TIER 2: DAILY OPERATIONS, INBOUND QUESTIONNAIRES & ROUTINE BROADCASTS */}
          <div className="bg-slate-50/95 border border-slate-200/90 rounded-2xl p-2 shadow-2xs flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2 overflow-x-auto no-scrollbar">
            
            {/* Right Wing in RTL (מימין): Inbound Questionnaires & Scanner */}
            <div className="flex items-center gap-2 overflow-x-auto no-scrollbar shrink-0">
              {/* Review Intake Questionnaires */}
              <button
                type="button"
                onClick={() => {
                ...
                className={`font-bold px-3 py-1.5 rounded-xl text-xs sm:text-sm flex items-center gap-1.5 transition-all cursor-pointer shadow-2xs shrink-0 active:scale-95 border ${
                  newIntakeCount > 0
                    ? 'bg-rose-50 border-rose-400 text-rose-950 font-black ring-2 ring-rose-300/40'
                    : 'bg-white hover:bg-slate-100 border-slate-200 text-slate-700'
                }`}
                title="צפייה בשאלוני קליטה חדשים מלקוחות"
              >
                <span className="text-base">📥</span>
                <span>שאלונים לבדיקה</span>
                <span className={`text-[10px] font-mono font-black px-1.5 py-0.2 rounded-full ${
                  newIntakeCount > 0 ? 'bg-rose-600 text-white' : 'bg-slate-100 text-slate-600'
                }`}>
                  {newIntakeCount}
                </span>
              </button>
              ...
              <button
                ...
                title="צפייה בשאלונים פעילים הנמצאים בתהליך טיפול"
              >
                <span className="text-base">⏳</span>
                <span>שאלונים בתהליך</span>
                <span className={`text-[10px] font-mono font-black px-1.5 py-0.2 rounded-full ${
                  inProgressIntakeCount > 0 ? 'bg-amber-500 text-white' : 'bg-slate-100 text-slate-600'
                }`}>
              ...
              <button
                ...
                className="hidden lg:flex items-center gap-1.5 bg-white hover:bg-indigo-50 border border-indigo-200 text-indigo-950 font-bold px-3 py-1.5 rounded-xl text-xs shadow-2xs transition-all cursor-pointer shrink-0"
                title="סריקת וואטסאפ במחשב (מוגן)"
              >
                <span className="text-base">🔍🖥️</span>
            ...
            <div className="flex items-center justify-end gap-2 overflow-x-auto no-scrollbar shrink-0">
              {/* Pending Payment Button (if any) */}
              {pendingGrowPayments.length > 0 && (
                <button
                  ...
                  className="bg-emerald-600 hover:bg-emerald-700 text-white font-black px-3 py-1.5 rounded-xl text-xs flex items-center gap-1.5 shadow-2xs transition-all cursor-pointer shrink-0 animate-pulse"
                  title="תשלום ממתין להקמת הזמנה"
                >
                  <span>💳 תשלום ממתין: ₪{pendingGrowPayments[0]?.amount.toLocaleString()}</span>
                </button>
              )}

              {/* Daily Evening Dog Update (20:00) */}
              <button
                type="button"
              ...
              <button
                ...
                className="bg-gradient-to-r from-indigo-50 to-blue-50 hover:from-indigo-100 hover:to-blue-100 active:scale-95 border border-indigo-300 text-indigo-950 font-black px-3.5 py-1.5 rounded-xl text-xs sm:text-sm shadow-2xs flex items-center gap-1.5 transition-all cursor-pointer shrink-0"
                title="מה קורה מחר? סקירה יומית מלאה לשמוליק"
              >
                <span className="text-base">📋</span>
